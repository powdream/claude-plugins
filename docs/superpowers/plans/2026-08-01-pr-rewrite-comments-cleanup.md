...
for p in sorted(pathlib.Path("plugins").rglob("SKILL.md")):
    ...
    name, desc = n.group(1).strip(), d.group(1).strip()
...
for p in mk["plugins"]:
    if p["name"] not in on_disk:
        print("PRE-EXISTING STALE ENTRY (not this plan's to fix):", p["name"])
        continue
...
Expected: `PRE-EXISTING STALE ENTRY (not this plan's to fix): invoke-agent`
followed by `OK 7 plugins on disk`.

`invoke-agent` is registered in `.claude-plugin/marketplace.json` but has no
directory under `plugins/`. That defect predates this plan and is out of scope —
do not fix it here, and do not let it block the task.
